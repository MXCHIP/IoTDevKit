...
DIGITALTWIN_INTERFACE_CLIENT_HANDLE SettingsInterface_Create()
{
    ...
    if ((result = DigitalTwin_InterfaceClient_Create(SettingsInterfaceId,  SettingsComponentName, SettingsInterface_InterfaceRegisteredCallback, (void*)&appState, &interfaceHandle)) != DIGITALTWIN_CLIENT_OK)
    {
        LogError("SETTINGS_INTERFACE: Unable to allocate interface client handle for interfaceId=<%s>, componentName=<%s>, error=<%s>", SettingsInterfaceId, SettingsComponentName, MU_ENUM_TO_STRING(DIGITALTWIN_CLIENT_RESULT, result));
    ...
    {
        LogInfo("SETTINGS_INTERFACE: Created DIGITALTWIN_INTERFACE_CLIENT_HANDLE successfully for interfaceId=<%s>, componentName=<%s>, handle=<%p>", SettingsInterfaceId, SettingsComponentName, interfaceHandle);
