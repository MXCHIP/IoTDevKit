...
DIGITALTWIN_INTERFACE_CLIENT_HANDLE ScreenInterface_Create()
{
    ...
    if ((result = DigitalTwin_InterfaceClient_Create(ScreenInterfaceId,  ScreenComponentName, ScreenInterface_InterfaceRegisteredCallback, (void*)&appState, &interfaceHandle)) != DIGITALTWIN_CLIENT_OK)
    {
        LogError("SCREEN_INTERFACE: Unable to allocate interface client handle for interfaceId=<%s>, componentName=<%s>, error=<%s>", ScreenInterfaceId, ScreenComponentName, MU_ENUM_TO_STRING(DIGITALTWIN_CLIENT_RESULT, result));
    ...
    {
        LogInfo("SCREEN_INTERFACE: Created DIGITALTWIN_INTERFACE_CLIENT_HANDLE successfully for interfaceId=<%s>, componentName=<%s>, handle=<%p>", ScreenInterfaceId, ScreenComponentName, interfaceHandle);
