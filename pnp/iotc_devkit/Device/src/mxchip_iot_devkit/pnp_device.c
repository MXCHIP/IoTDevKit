...
#define MODELDEFINITION_INDEX 1

#define SENSORS_INDEX 2
...
int pnp_device_initialize(const char* connectionString, const char* trustedCert)
{
    ...
    if ((interfaceClientHandles[MODELDEFINITION_INDEX] = ModelDefinitionInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
    ...
    if ((interfaceClientHandles[SYSTEM_INDEX] = SystemInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
        LogError("SystemInterface_Create failed");
        return -1;
    }
...
void pnp_device_close()
{
    ...
    if (interfaceClientHandles[MODELDEFINITION_INDEX] != NULL)
    {
        ModelDefinitionInterface_Close(interfaceClientHandles[MODELDEFINITION_INDEX]);
    }
    ...
    if (interfaceClientHandles[SYSTEM_INDEX] != NULL)
    {
        SystemInterface_Close(interfaceClientHandles[SYSTEM_INDEX]);
    }
